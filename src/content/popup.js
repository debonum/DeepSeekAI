...
} from "./components/ShadowContainer";
...
const getPopupInitialStyle = (rect) => ({
  ...
  boxShadow:
    "0 0 0 0.5px rgba(0, 0, 0, 0.05), 0 2px 8px rgba(0, 0, 0, 0.06), 0 4px 16px rgba(0, 0, 0, 0.08)",
...
export function createPopup(
  ...
  // 设置滚动处理器
  const cleanupScrollHandlers = setupScrollHandlers(
  ...
  const onGenerationComplete = () => {
    if (initialAnswerElement) {
      ...
      const originalColor =
        getComputedStyle(initialAnswerElement).backgroundColor;
      initialAnswerElement.style.backgroundColor =
  ...
  const questionInputContainer = createQuestionInputContainer(
    window.aiResponseContainer,
  );
  ...
  popup.addEventListener("mousedown", (e) => {
    if (
      ...
      e.target.closest(".expandable-textarea") ||
  ...
  interact(popup).resizable({
    ...
    modifiers: [
      interact.modifiers.restrictSize({
        min: {
          width: STYLE_CONSTANTS.MIN_WIDTH,
          height: STYLE_CONSTANTS.MIN_HEIGHT,
        },
        max: {
          width: STYLE_CONSTANTS.MAX_WIDTH,
          height: STYLE_CONSTANTS.MAX_HEIGHT,
        },
      }),
    ],
    listeners: {
      move: (event) => {
        ...
        if (event.deltaRect.width !== 0 || event.deltaRect.height !== 0) {
          // 限制反馈频率以避免过度震动
          if (
            !popup._lastFeedbackTime ||
            Date.now() - popup._lastFeedbackTime > 100
          ) {
            provideTactileFeedback();
...
function updateScroll({ width, height, ps }) {
  ...
  const resizeObserver = new ResizeObserver((entries) => {
    for (const entry of entries) {
      ...
      requestAnimationFrame(() => {
        const visibleIconContainer = window.aiResponseContainer.querySelector(
          '.icon-container[style*="display: flex"]',
        );
        if (visibleIconContainer) {
          const containerRect =
            window.aiResponseContainer.getBoundingClientRect();
          const iconRect = visibleIconContainer.getBoundingClientRect();

          if (iconRect.bottom > containerRect.bottom) {
            window.aiResponseContainer.scrollTop +=
              iconRect.bottom - containerRect.bottom + 10;
          }
        }
