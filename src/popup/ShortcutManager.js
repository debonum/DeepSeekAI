...
export class ShortcutManager {
  ...
  validateShortcut(shortcutString, command = 'toggle-chat') {
    ...
    // 检查是否只使用了标准修饰键
    const modifiers = [shortcut.ctrl, shortcut.shift, shortcut.alt, shortcut.meta];
    if (modifiers.filter(Boolean).length === 0) {
      return { valid: false, error: '至少需要一个修饰键（Ctrl、Shift、Alt 或 Cmd）' };
    }

    // 检查操作系统特定的限制
    const isMac = navigator.platform.toUpperCase().includes('MAC');
    if (isMac && shortcut.ctrl && !shortcut.meta) {
      return { valid: false, error: '在 macOS 上建议使用 Cmd 键而不是 Ctrl 键' };
    }
  ...
  attachShortcutModalEvents(modal) {
    ...
    resetButton.addEventListener('click', () => {
      shortcutInput.value = this.formatShortcutForDisplay(this.parseShortcut(this.defaultShortcuts['toggle-chat'].default));
