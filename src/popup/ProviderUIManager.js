export class ProviderUIManager {
  ...
  async updateProviderUI(provider) {
    ...
    try {
      ...
      const url =
        this.providerManager.getApiKeyLink(provider) ||
        this.providerManager.getApiKeyLink("deepseek");
      ...
      this.providerManager
        ...
        .then((key) => {
          ...
          if (addModelLink) {
            ...
            addModelLink.style.display =
              isCustom || isAddCustom || hasSavedKey ? "none" : "inline";
          }
        })
        .catch(() => {
          ...
          if (addModelLink) {
            addModelLink.href = url;
            addModelLink.style.display =
              isCustom || isAddCustom ? "none" : "inline";
          }
        });
    } catch (e) {}
  ...
  async loadCustomApiUrl(provider) {
    try {
      ...
      if (
        customApiUrl &&
        this.uiManager.elements.customApiUrl &&
        !this.uiManager.elements.customApiUrl.value
      ) {
        this.uiManager.elements.customApiUrl.value = customApiUrl;
      }

      console.log(
        `加载${provider}的API URL: 自定义=${customApiUrl}, 默认=${defaultUrl}`,
      );
    } catch (error) {
  ...
  async createCustomProviderDropdown(currentProvider) {
    ...
    const providerContainer = providerSelect.closest(
    ...
    if (providerContainer) {
      ...
      // 移除之前可能存在的点击事件
      providerContainer.removeEventListener(
        "click",
        this.providerSelectClickListener,
      );
      ...
      providerContainer.addEventListener(
        "click",
        this.providerSelectClickListener,
      );
    }
    ...
    providers.forEach((provider, index) => {
      ...
      deleteBtn.title = this.i18nManager.getTranslation(
        "deleteProviderBtnTitle",
      );
      deleteBtn.innerHTML =
      ...
      if (
        ...
        provider.id.startsWith("custom_") !==
          providers[index + 1].id.startsWith("custom_")
      ) {
        addDivider();
      }
    });
    ...
    const documentClickListener = (e) => {
      if (
        e.target !== providerSelect &&
        !providerSelect.contains(e.target) &&
        e.target !== customDropdown &&
        !customDropdown.contains(e.target) &&
        e.target !== providerContainer &&
        !providerContainer.contains(e.target)
      ) {
        customDropdown.style.display = "none";
      }
    };
  ...
  cleanupCustomProviderDropdown() {
    try {
      ...
      if (this.providerSelectClickListener) {
        const providerContainer = document
      ...
      if (this.customProviderDropdown && this.dropdownClickListener) {
        this.customProviderDropdown.removeEventListener(
          "click",
      ...
      if (customDropdown) {
        // 获取所有选项
        const options = customDropdown.querySelectorAll(
        ...
        // 获取所有删除按钮并移除事件监听器
        const deleteButtons = customDropdown.querySelectorAll(
          ".delete-provider-btn",
        );
  ...
  showDeleteProviderDialog(providerId, providerName) {
    ...
    const deleteProviderConfirmText = document.getElementById(
      "deleteProviderConfirmText",
    );
    const confirmDeleteProviderButton = document.getElementById(
      "confirmDeleteProviderButton",
    );
    const cancelDeleteProviderButton = document.getElementById(
      "cancelDeleteProviderButton",
    );
    const closeDeleteProviderModal = document.getElementById(
      "closeDeleteProviderModal",
    );
    const deleteProviderValidationMessage = document.getElementById(
    ...
    if (deleteProviderTitle) {
      deleteProviderTitle.textContent =
        this.i18nManager.getTranslation("deleteProvider");
    }

    // 设置确认文本
    if (deleteProviderConfirmText) {
      const confirmText = this.i18nManager
        .getTranslation("deleteProviderConfirm")
    ...
    if (confirmDeleteProviderButton) {
      confirmDeleteProviderButton.textContent =
        this.i18nManager.getTranslation("deleteProvider");
    }
  ...
  showCustomProviderDialog() {
    ...
    const customProviderValidationMessage = document.getElementById(
    ...
    const tempState = this.tempStateManager?.getTempState?.(
      this.tempStateManager.constructor.TYPES.ADD_PROVIDER,
    );
    ...
    // 设置输入框的placeholder
    if (customProviderNameInput)
      customProviderNameInput.placeholder = this.i18nManager.getTranslation(
        "customProviderNameExamplePlaceholder",
      );
    if (customProviderApiKey)
      customProviderApiKey.placeholder = this.i18nManager.getTranslation(
        "customProviderApiKeyPlaceholder",
      );
    if (customApiUrlInput)
      customApiUrlInput.placeholder = this.i18nManager.getTranslation(
        "customProviderUrlExamplePlaceholder",
      );
    if (customModelIdInput)
      customModelIdInput.placeholder = this.i18nManager.getTranslation(
        "customProviderModelNameExamplePlaceholder",
      );
    if (customModelNameInput)
      customModelNameInput.placeholder = this.i18nManager.getTranslation(
  ...
  async handleDeleteProvider() {
    ...
    try {
      ...
      // 删除服务商
      const success = await this.providerManager.deleteCustomProvider(
      ...
      if (success) {
        ...
        if (
          currentProvider === this.providerToDelete ||
        ...
        // 显示成功消息
        if (validationMessage) {
          validationMessage.innerHTML = this.i18nManager.getTranslation(
  ...
  async handleSaveCustomProvider() {
    ...
    const saveCustomProviderButton = document.getElementById(
    ...
    if (!customProviderName) {
      ...
      customProviderValidationMessage.innerHTML =
        this.i18nManager.getTranslation("customProviderEmpty");
      customProviderValidationMessage.classList.add("error");
      return;
    }
    ...
    if (!customModelId) {
      ...
      customProviderValidationMessage.innerHTML =
        this.i18nManager.getTranslation("customModelIdEmpty");
      customProviderValidationMessage.classList.add("error");
      return;
    }
    ...
    if (!customModelName) {
      ...
      customProviderValidationMessage.innerHTML =
        this.i18nManager.getTranslation("customModelNameEmpty");
      customProviderValidationMessage.classList.add("error");
      return;
    }
    ...
    try {
      ...
      // 验证API密钥
      const isValid = await this.providerManager.validateApiKey(
        "temp_provider",
        apiKey,
        customModelId,
      );
      ...
      // 移除临时服务商
      this.providerManager.customProviders =
        this.providerManager.customProviders.filter(
          (p) => p.id !== "temp_provider",
        );
      ...
      if (isValid) {
        ...
        // 保存自定义API URL
        await this.providerManager.saveCustomApiUrl(
          newProvider.id,
          customApiUrl,
        );
        ...
        customProviderValidationMessage.innerHTML =
          this.i18nManager.getTranslation("customProviderSaveSuccess");
